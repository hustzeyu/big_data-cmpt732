...
def main(inputs, output):
    ...
    broad = sc.broadcast(dict(positive_average.collect()))

    commentbysub = commentdata.map(lambda c: (c['subreddit'], c))
    outdata = commentbysub.map(lambda c: (rltv_scr(c, broad))).sortBy(get_key, ascending=False)
